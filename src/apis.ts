...
export const apis = {
	user: {
		check: (uid: string) => axios.get<ApiResponse>(`${server}/api/users/is_active?uid=${uid}`),
		authorize: (login: string, password: string) =>
			axios.post<ApiResponse>(
				`${server}/api/users/auth`,
				{login: login, password: password}
			)
	},
	branches: {
		list: () => axios.get<ApiResponse>(`${server}/api/branches/`),
		get: (id: string) => axios.get<ApiResponse>(`${server}/api/branches?id=${id}`)
	},
	product: {
		get: (id: string) => axios.get<ApiResponse>(`${server}/api/products?id=${id}`),
		owned: (uid: string) => axios.get<ApiResponse>(`${server}/api/products/owned?uid=${uid}`)
